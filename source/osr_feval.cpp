...
bool OSRFeval::processCompVersion(JITCompiler::CompFunction* pCompFunction, JITCompiler::CompVersion* pCompVersion) {
    ...
    for (std::pair<FevalInfoForOSR*, llvm::BasicBlock*> &splitPair: splitInfoVec) {
        ...
        LivenessAnalysis::LiveValues& liveValues = livenessInfo.getLiveInValues(splitBB);

        std::cerr << "Live Value* for instruction " << info->pParamExpr->toString() << std::endl;
        for (const llvm::Value* v: liveValues) {
